...
void get_time(unsigned char *clock_register)
{
    clock_register[0] = read_ds1307(0x02);
    clock_register[1] = read_ds1307(0x01);
    clock_register[2] = read_ds1307(0x00);
}
void store_events(void)
{
    if(ADDR == 120)
...
void Dash_board(void)
{
    ...
    else if(key == 0x37)
    {
            ...
            clcd_print(events[Event_index], (0xC0 + 9));
...
void main_menu()
{
    ...
    clcd_putch('*', (0x80 + 1));
    if(main_index < 3 && key == 0x3E)
    {
         menu_count++;
         main_index++;
         clcd_write(0x01, 0);
    }
    else if(main_index > 0 && key == 0x3D)
    {
         menu_count--;
         main_index--;
         clcd_write(0x01, 0);
    ...
    {
        clcd_print(Main_menu[main_index], (0x80 + 3));
        clcd_print(Main_menu[main_index+1], (0xC0 + 3));
    }
}
void view_log(void)
{
    ...
    if (clear_flag == 0 && event_count > 0)
    {
        ...
        while (1)
        {
            ...
            clcd_print(time_data, (0xC0 + 2));
            clcd_print(event_data, (0xC0 + 11));
            clcd_print(speed_data, (0xC0 + 14));
    ...
    else
    {
        ...
        event_count = 0;
        clcd_write(0x01, 0);
        clcd_print("NO_LOGS(EMPTY)", (0x80 + 2));
       _delay((unsigned long)((410000)*(20000000/4000000.0)));
        clcd_write(0x01, 0);
        main_flag = MAIN_MENU;
    }
}
...
void set_time(void)
{
    ...
    while(1)
    {
        ...
        {
            write_ds1307(0x02, Clock_register[0]);
            write_ds1307(0x01, Clock_register[1]);
            write_ds1307(0x00, Clock_register[2]);
            clcd_write(0x01, 0);
            main_flag = MAIN_MENU;
            break;
        }
